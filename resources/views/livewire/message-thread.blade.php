        ...
        <div class="ml-3 flex shrink-0 items-center gap-2">
            ...
            @if(in_array($role, ['admin', 'evaluator'], true))
                @if($conversation->status === 'open')
                    {{-- Roadmap #35 — recordar al editor por correo (con reglas anti-spam). --}}
                    <button wire:click="remindEditor" wire:loading.attr="disabled"
                            class="flex items-center gap-1.5 rounded-md bg-amber-50 px-3 py-1.5 text-xs font-medium text-amber-700 transition hover:bg-amber-100 disabled:opacity-50 dark:bg-amber-900/30 dark:text-amber-300 dark:hover:bg-amber-900/50">
