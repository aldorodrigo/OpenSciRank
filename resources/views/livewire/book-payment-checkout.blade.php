...
<div class="bg-gray-50 py-8 dark:bg-gray-950">
    <div class="container mx-auto max-w-5xl px-4">
        ...
        <nav class="mb-6 text-sm text-gray-500 dark:text-gray-400">
            ...
            <span class="text-gray-900 dark:text-white">{{ __('Book Listing Payment') }}</span>
        </nav>

        <h1 class="mb-8 text-3xl font-bold text-gray-900 dark:text-white">{{ __('Payment for Book Listing') }}</h1>
            ...
            <div class="lg:col-span-2 space-y-6">
                        ...
                        @foreach($this->availablePlans as $product)
                            <div wire:click="selectPlan({{ $product->id }})"
                                class="relative cursor-pointer rounded-xl border-2 p-6 transition
                                    @if($selectedPlan === $product->id) border-brand bg-blue-50 dark:border-blue-500 dark:bg-blue-900/20
                                ...
                                <div class="mb-4 flex items-center justify-between">
                                    ...
                                    <div class="flex h-5 w-5 items-center justify-center rounded-full border-2
                                        @if($selectedPlan === $product->id) border-brand bg-brand
                ...
                <div class="rounded-xl bg-white p-6 shadow-lg dark:bg-gray-900">
                    ...
                    <div class="flex items-center gap-4 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-brand" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-24 rounded-xl bg-white p-6 shadow-lg dark:bg-gray-900">
                    ...
                    <div class="flex justify-between py-4">
                        ...
                        <span class="text-lg font-bold text-brand dark:text-blue-400">${{ number_format($this->total, 2) }} {{ $this->products->firstWhere('id', $selectedPlan)?->currency }}</span>
                    </div>

                    <button wire:click="processPayment"
                        wire:loading.attr="disabled"
                        class="w-full rounded-lg bg-brand py-3 text-center font-semibold text-white shadow-sm transition hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                    ...
                    <p class="mt-4 text-center text-xs text-gray-500 dark:text-gray-400">
                        {{ __('By making the payment you accept our') }} <a href="/terms" class="text-brand hover:underline">{{ __('Terms of Service') }}</a>.
                    </p>
                </div>
            </div>
        </div>
