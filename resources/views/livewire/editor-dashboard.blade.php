...
<div class="bg-gray-50 py-8 dark:bg-gray-950"
    ...
    <div class="container mx-auto px-4">
        ...
        @if(auth()->user()?->primaryPanelRole() === 'evaluator')
            @php
                $evaluatorPending = \App\Models\AdminTask::where('assigned_to', auth()->id())
                    ->whereIn('status', \App\Models\AdminTask::STATUSES_WORK_QUEUE)
                    ->count();
            @endphp
            <a href="{{ route('filament.admin.pages.evaluator-desk') }}"
               class="mb-8 flex flex-col items-start justify-between gap-4 rounded-2xl border border-amber-200 bg-gradient-to-r from-amber-50 to-white p-5 shadow-sm transition hover:border-amber-300 hover:shadow sm:flex-row sm:items-center dark:border-amber-800/60 dark:from-amber-900/20 dark:to-gray-900">
                <div class="flex items-center gap-4">
                    <span class="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">
                        ...
                        @if($evaluatorPending > 0)
                            <span class="absolute -right-1.5 -top-1.5 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-amber-600 px-1 text-xs font-bold text-white">{{ $evaluatorPending > 99 ? '99+' : $evaluatorPending }}</span>
                        @endif
                    </span>
                    <div>
                        ...
                        <p class="mt-0.5 text-sm text-gray-600 dark:text-gray-400">
                            {{ $evaluatorPending > 0
                                ? trans_choice('evaluator_access.banner_pending', $evaluatorPending, ['count' => $evaluatorPending])
                                : __('evaluator_access.banner_body') }}
                        </p>
                    </div>
                </div>
            ...
            @if($allJournals->isEmpty())
                <div class="p-8">
                    ...
                    <div class="mx-auto mt-6 grid max-w-2xl gap-4 sm:grid-cols-2">
                        ...
                        <div class="rounded-xl border-2 border-blue-200 bg-blue-50/50 p-5 text-center dark:border-blue-800 dark:bg-blue-950/20">
                                        ...
                                        <div x-data="{ open: false, top: '0px', left: '0px' }" @click.outside="open = false">
                                            ...
                                            <div x-show="open" x-transition
                                                ...
                                                <div class="py-1">
                                                    ...
                                                    {{-- Sprint 3 #20: comprar/extender el destacado para libros ya listed --}}
                                                    @if($book->status === 'listed')
                                                        <a href="{{ route('app.book.checkout', $book) }}"
                                                            class="flex items-center gap-2 px-4 py-2 text-sm text-amber-600 hover:bg-gray-50 dark:text-amber-400 dark:hover:bg-gray-700">
                                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118L2.05 9.10c-.783-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.518-4.674z"/></svg>
