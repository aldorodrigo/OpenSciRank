<x-layouts.app title="{{ $post->title }} - Blog" description="{{ $post->excerpt }}">
            ...
            <div class="mx-auto max-w-3xl">
                ...
                <header class="mb-8">
                    <div class="flex items-center gap-3 mb-4">
                        <span class="rounded-full {{ $post->cat_color }} px-3 py-1 text-xs font-semibold">{{ $post->cat_label }}</span>
                        @if($post->is_featured)
                    ...
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white sm:text-4xl lg:text-5xl leading-tight">{{ $post->title }}</h1>
                ...
                <a href="{{ route('blog.show', $relatedPost->slug) }}" class="group flex flex-col overflow-hidden rounded-xl bg-white shadow-sm transition hover:shadow-lg dark:bg-gray-950">
                    <div class="flex h-48 items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700">
                        @if($relatedPost->image_path)
                            <img src="{{ Storage::url($relatedPost->image_path) }}" alt="{{ $relatedPost->title }}" class="h-full w-full object-cover">
                        @else
                            <span class="text-6xl">{{ $relatedPost->emoji ?? '📝' }}</span>
                        @endif
                    </div>
                    <div class="flex flex-1 flex-col p-6">
                        <span class="inline-flex w-fit rounded-full {{ $relatedPost->cat_color }} px-3 py-1 text-xs font-semibold">{{ $relatedPost->cat_label }}</span>
                        <h3 class="mt-3 flex-1 font-bold text-gray-900 transition group-hover:text-indigo-600 dark:text-white dark:group-hover:text-indigo-400">{{ $relatedPost->title }}</h3>
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400 line-clamp-2">{{ $relatedPost->excerpt }}</p>
