<x-filament-panels::page>
    <div x-data="{
                ...
                <div class="flex flex-col items-start justify-between gap-8 lg:flex-row lg:items-center">
                    ...
                    <div class="flex-1 space-y-4">
                        ...
                        <div class="flex flex-wrap gap-2 pt-2">
                            {{-- Roadmap #35 — ficha pública de la revista. --}}
                            @if(filled($record->slug))
                                <a href="{{ route('journal.show', ['slug' => $record->slug]) }}" target="_blank" class="flex items-center gap-2 rounded-lg bg-emerald-100 px-3 py-1.5 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200 transition hover:bg-emerald-200 dark:bg-emerald-500/15 dark:text-emerald-300 dark:ring-emerald-500/30 dark:hover:bg-emerald-500/25">
        ...
        {{-- ── Roadmap #35: Consultar al editor (mensajería anclada al Journal) ── --}}
        <div class="mt-8 rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
            <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('evaluator_msg.section_title') }}</h2>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{{ __('evaluator_msg.section_desc') }}</p>
            </div>
            ...
            @if($this->editorConversation)
                <div style="height: 520px; display: flex; flex-direction: column; overflow: hidden;">
                    @livewire('message-thread', [
                        'conversation' => $this->editorConversation,
                        'role'         => $this->messagingRole,
                    ], key('journal-conv-'.$this->editorConversation->id))
                </div>
            @else
                <div class="flex flex-col items-center justify-center gap-4 py-12 text-center">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('evaluator_msg.section_empty') }}</p>
                    <button wire:click="startEditorConversation" wire:loading.attr="disabled"
                            class="inline-flex items-center gap-2 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-500 disabled:opacity-60">
                        {{ __('evaluator_msg.start_button') }}
                    </button>
                </div>
            @endif
        </div>
