<x-layouts.app :title="__('Directory - Editorial Standards Platform')" :description="__('Explore the directory of scientific journals and academic books on Editorial Standards Platform. Filter by status, country, discipline and more.')">
    ...
    <section class="bg-brand-deep py-14 text-white">
        <div class="container mx-auto px-4 text-center">
            ...
            <p class="mx-auto mt-4 max-w-2xl text-blue-200">{{ __('Explore scientific journals and academic books. Editorial evaluation based on transparent criteria.') }}</p>
            ...
            <div class="mt-8 flex flex-wrap items-center justify-center gap-8">
                <div class="text-center">
                    ...
                    <div class="mt-1 text-sm text-blue-200">{{ __('Journals in the directory') }}</div>
                </div>
                <div class="h-10 w-px bg-white/20"></div>
                <div class="text-center">
                    <div class="text-3xl font-extrabold">{{ number_format($bookCount) }}</div>
                    <div class="mt-1 text-sm text-blue-200">{{ __('Indexed books') }}</div>
                </div>
                <div class="h-10 w-px bg-white/20"></div>
                <div class="text-center">
                    <div class="text-3xl font-extrabold">{{ number_format($certifiedCount) }}</div>
                    <div class="mt-1 text-sm text-blue-200">{{ __('Certified Journals') }}</div>
                </div>
                <div class="h-10 w-px bg-white/20"></div>
                <div class="text-center">
                    <div class="text-3xl font-extrabold">5</div>
                    <div class="mt-1 text-sm text-blue-200">{{ __('Evaluation areas') }}</div>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="border-t border-gray-200 bg-white py-14 dark:border-gray-800 dark:bg-gray-900">
        <div class="container mx-auto px-4 text-center">
            ...
            <a href="/register" class="mt-8 inline-flex items-center gap-2 rounded-lg bg-brand px-8 py-3 font-semibold text-white shadow-sm transition hover:bg-blue-500">
